...
KEY_0:
      		//Toggle the value of RUN whenever key0 is pressed
      		LDR R0, RUN
      		EOR R0, #1
      		STR R0, RUN
...
KEY_1: 
      		//Speed should be doubled, meaning half the timer value 
      		LDR R0, =TIMER_BASE
      		LDR R1, [R0, #0x8] 		//Get the lower count
      		LDR R2, [R0,#0xC] 		//Get the higher count
...
KEY_2:
    		//Speed should slow down by half, meaning double the timer value ->2FAF080
      		LDR R0, =TIMER_BASE
      		LDR R1, [R0, #0x8] 		//Get the lower count
      		LDR R2, [R0,#0xC] 		//Get the higher count
